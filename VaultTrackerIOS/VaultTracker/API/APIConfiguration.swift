...
enum APIEnvironment {
    ...
    var baseURL: String {
        ...
        case .production:
            return "https://vaulttracker-api.onrender.com"
        }
    }
}
...
enum APIConfiguration {
    ...
    /// `DEBUG` builds use local/LAN (`API_HOST` scheme variable) so integration tests and dev servers work.
    /// `RELEASE` archives talk to production on Render.
    #if DEBUG
    ...
    enum Endpoints {
        ...
        /// POST /api/v1/transactions/smart — server resolves account + asset
        static let smartTransaction = "\(apiVersion)/transactions/smart"

        // MARK: Analytics

        /// GET /api/v1/analytics
        static let analytics = "\(apiVersion)/analytics"

        // MARK: Prices

        /// POST /api/v1/prices/refresh
        static let priceRefresh = "\(apiVersion)/prices/refresh"
