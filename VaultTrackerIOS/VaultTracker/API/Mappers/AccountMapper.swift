...
enum AccountMapper {
    ...
    /// Inline account summary from an enriched transaction row.
    @MainActor
    static func toDomain(_ summary: APIAccountSummary) -> Account {
        Account(
            id: UUID(uuidString: summary.id) ?? UUID(),
            name: summary.name,
            accountType: mapAccountType(summary.accountType),
            creationDate: Date()
        )
    }
