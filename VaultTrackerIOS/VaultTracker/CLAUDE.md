...
- **Unit tests** use Swift Testing (`@Test`, `#expect`). ViewModel coverage includes `HomeViewModelTests`, `AnalyticsViewModelTests`, and `AddAssetFormViewModelTests` (validation / request encoding without the network). `AuthManagerTests` injects `FakeFirebaseAuthBackend` + isolated `NotificationCenter` (optional `VTLogging`; default `VTLog.shared`) for auth state, debug sign-out, and `.authenticationRequired` behavior. `APIServiceTests` builds the service with `APIService.test_make(session:log:)`, `URLSession` + stub `URLProtocol`, `VTLoggingSpy`, and `APIServiceTestAuth.withDebugSession` / `AuthTokenProvider.forceTokenRefreshFailure` where needed (the `APIService` suite’s `@Test` bodies are under `#if DEBUG` because they touch `isDebugSession`). `AuthTokenProviderTests` uses `AuthTokenProvider.test_make(log:)` without file-level `#if DEBUG`.
