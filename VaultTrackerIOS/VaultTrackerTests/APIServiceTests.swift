...
#if DEBUG
/// `AuthTokenProvider.isDebugSession` exists only in DEBUG; unit tests use it to avoid Firebase.
private enum APIServiceTestAuth {
    static func withDebugSession<T>(_ body: () async throws -> T) async rethrows -> T {
        let previous = AuthTokenProvider.isDebugSession
        AuthTokenProvider.isDebugSession = true
        defer { AuthTokenProvider.isDebugSession = previous }
...
extension APIServiceTests {
    ...
    private func makeService(log: any VTLogging) -> APIService {
        let config = URLSessionConfiguration.ephemeral
        config.protocolClasses = [StubURLProtocol.self]
        let session = URLSession(configuration: config)
        return APIService.test_make(session: session, log: log)
    }

    @Test func fetchDashboardSuccessLogsAndDecodes() async throws {
        try await APIServiceTestAuth.withDebugSession {
    ...
    @Test func tokenRefreshFailureAfter401LogsNotAuthenticated() async throws {
        try await APIServiceTestAuth.withDebugSession {
            let previousFail = AuthTokenProvider.forceTokenRefreshFailure
            AuthTokenProvider.forceTokenRefreshFailure = true
            defer { AuthTokenProvider.forceTokenRefreshFailure = previousFail }
