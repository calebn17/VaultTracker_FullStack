...
- **Secrets and git** — `VaultTrackerAPI/.env` and the iOS Firebase client plist (`GoogleService-info.plist`) must remain **untracked**. Use `VaultTrackerAPI/.env.example` and `VaultTrackerIOS/VaultTracker/GoogleService-info.plist.example` as templates. Rotate any credentials that may have appeared in git history.
