...
5. **TanStack Query:** `ApiClientProvider` calls `queryClient.clear()` when `AuthContext.user` is `null` (sign-out or session cleared) and before `signOutUser` / redirect when the API reports a persistent 401, so cached server data from the previous account cannot flash after another user signs in.
