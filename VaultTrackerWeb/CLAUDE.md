...
5. If `getToken` throws (no session or refresh failure), `ApiClient` calls the same `onUnauthorized` path as a persistent 401 so the user is redirected to `/login` instead of staying on an authenticated route in a broken state.
6. **TanStack Query:** `ApiClientProvider` calls `queryClient.clear()` when `AuthContext.user` is `null` (sign-out or session cleared) and before `signOutUser` / redirect when the API reports a persistent 401, so cached server data from the previous account cannot flash after another user signs in.
...
**Security headers:** `next.config.ts` defines `headers()` on the inner config (wrapped by `withSentryConfig`) — CSP plus standard browser hardening headers for `/:path*`. After changing third-party scripts or API hosts, run `npm run build`, `npm run start`, and confirm the browser console has no CSP violations on login and main flows. **CSP note:** production `script-src` currently includes `'unsafe-inline'` because Next.js App Router still relies on inline scripts for hydration/RSC; tightening to nonce-based `script-src` would need middleware-issued nonces and aligned `next/script` usage — documented as a known gap in `next.config.ts` until implemented.
