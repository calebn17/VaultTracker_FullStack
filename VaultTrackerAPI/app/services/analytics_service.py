...
class AnalyticsService:
    ...
    def get_household_analytics(self, household_id: str, db: Session) -> dict:
        member_ids = [
            m.user_id
        ...
        transactions = (
            db.query(Transaction).filter(Transaction.user_id.in_(member_ids)).all()
        )
        total_invested = sum(
            t.quantity * t.price_per_unit
            for t in transactions
